...
class EmailMessageResource(Resource):
  ...
  , API_NAME, API_VERSION, SCOPES)
    ...
    def post(self):
        ...
        parser.add_argument('customer_email', location='json')
        parser.add_argument('subject_email', location='json')
        parser.add_argument('message_email', location='json')
        args = parser.parse_args()
        
        emailMessage = EmailMessage(args['customer_email'], args['subject_email'], args['message_email'])
        db.session.add(emailMessage)
        db.session.commit()
        app.logger.debug('DEBUG: %s', args)

        return self.sendMessage(args['customer_email'], args['subject_email'], args['message_email']), 200 
